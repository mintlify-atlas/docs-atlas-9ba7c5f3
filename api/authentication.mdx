...
<Note>
Returns the authenticated user's information. Similar to OIDC's `/userinfo` endpoint.
</Note>

```bash GET /api/v1/auth/me
curl https://your-memos-instance.com/api/v1/auth/me \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```

### Request

No parameters needed. The access token in the `Authorization` header identifies the user.
...
```json
{
  "user": {
    ...
    "description": "Just testing Memos",
    "state": "NORMAL",
...
```bash POST /api/v1/auth/signout
curl -X POST https://your-memos-instance.com/api/v1/auth/signout \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -b cookies.txt
```
...
No parameters needed. The endpoint will:
1. Revoke the refresh token (from `memos_refresh` cookie)
2. Clear the authentication cookie
3. Invalidate the current session

<Info>
The access token in the `Authorization` header is still technically valid until expiry (15 minutes), but the refresh token is immediately revoked, preventing session renewal.
</Info>
...
The `memos_refresh` cookie is cleared:
```
Set-Cookie: memos_refresh=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0
```
...
curl -X POST "$BASE_URL/api/v1/auth/signout" \
  -H "Authorization: Bearer $ACCESS_TOKEN" \
  -c cookies.txt \
  -b cookies.txt
...
curl -X POST "$BASE_URL/api/v1/auth/refresh" \
  -b cookies.txt
# Returns: 401 Unauthenticated - refresh token not found
```
...
<Note>
Exchanges a valid refresh token for a new access token. The refresh token is read from the `memos_refresh` HttpOnly cookie.
</Note>

```bash POST /api/v1/auth/refresh
curl -X POST https://your-memos-instance.com/api/v1/auth/refresh \
  -b cookies.txt \
  -c cookies.txt
```
...
No request body needed. The refresh token is automatically read from the `memos_refresh` cookie.
...
A new `memos_refresh` cookie is set with a fresh 30-day expiry:
```
Set-Cookie: memos_refresh=<new_token>; Path=/; HttpOnly; SameSite=Lax; Expires=Sun, 30 Mar 2026 10:00:00 GMT
```
...
sequenceDiagram
    ...
    Client->>API: POST /api/v1/auth/refresh<br/>Cookie: memos_refresh=old_token
    API->>API: Parse refresh token JWT
    API->>DB: Validate token not revoked
    DB-->>API: Token valid
    API->>API: Generate new access token (15min)<br/>Generate new refresh token (30d)
    ...
    Note over Client: Old refresh token is now invalid
```
...
# Initial sign in
curl -s -c $COOKIES -X POST "$BASE_URL/api/v1/auth/signin" \
  -H "Content-Type: application/json" \
  -d '{
    "passwordCredentials": {
      "username": "demo",
...
curl "$BASE_URL/api/v1/memos" \
  -H "Authorization: Bearer $ACCESS_TOKEN"

# After 15 minutes, refresh token
REFRESH_RESPONSE=$(curl -s -b $COOKIES -c $COOKIES -X POST "$BASE_URL/api/v1/auth/refresh")
...
<ResponseField name="401" type="error">
  **Unauthenticated** - Refresh token missing, expired, or revoked
  
  Common causes:
  - No `memos_refresh` cookie sent
...
<Warning>
If refresh fails with 401, the user must sign in again. There is no way to recover an expired or revoked refresh token.
</Warning>
